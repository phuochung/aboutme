...
public class ControllerAdminDelSay extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ControllerAdminDelSay() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(("Member".equals(Ulogin.getRank()))||("Mod").equals(Ulogin.getRank())){
			response.sendRedirect(request.getContextPath()+"/admin/danhngon?msg=6");
			return;
		}
		ModelSaying mdSay = new ModelSaying();
		if(request.getParameter("idS")!=null){
			int idSay = Integer.valueOf(request.getParameter("idS"));
			if(mdSay.delSay(idSay)>0){
