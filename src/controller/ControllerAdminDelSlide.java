...
public class ControllerAdminDelSlide extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ControllerAdminDelSlide() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(("Member".equals(Ulogin.getRank()))||("Mod").equals(Ulogin.getRank())){
			response.sendRedirect(request.getContextPath()+"/admin/slide?msg=6");
			return;
		}
		ModelSlide mdSli = new ModelSlide();
		if(request.getParameter("idS")!=null){
			int idSli = Integer.valueOf(request.getParameter("idS"));
			if(mdSli.delSli(idSli)>0){
				response.sendRedirect(request.getContextPath()+"/admin/slide?msg=3");
				return;
			}else{
			...
			RequestDispatcher rd = request.getRequestDispatcher("/admin/slide");
