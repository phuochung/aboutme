...
public class ControllerPublicIndex extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String ipCliCheck = (String)session.getAttribute("ipClient");
		CountView cview =mdcv.getCountView();
		try {
			if(ipCliCheck.equals(ipClient)){
				//khong tang len
			}
			else{
		...
		session.setAttribute("ipClient", ipClient);
		
		request.setAttribute("objView", mdcv.getCountView());
